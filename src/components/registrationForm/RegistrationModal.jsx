...
const RegistrationModal = ({ isOpen, onClose, onRegistrationComplete }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      fetch('http://localhost/LiveOn-BloodDonationSystem/backend_api/controllers/get_hospitals.php', {
        credentials: 'include'
      })
        .then(res => res.json())
        .then(data => {
  ...
  const handleSubmitStep1 = async (e) => {
    ...
    if (Object.keys(errs).length === 0) {
      ...
      try {
        ...
        const response = await fetch('http://localhost/LiveOn-BloodDonationSystem/backend_api/controllers/register_donor.php', {
          method: 'POST',
  ...
  const handleSubmitStep2 = async (e) => {
    ...
    setIsSubmitting(true);
    try {
      const response = await fetch('http://localhost/LiveOn-BloodDonationSystem/backend_api/controllers/verify_otp.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          email: formData.email,
