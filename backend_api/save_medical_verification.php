...
class MedicalVerificationHandler
{
    private $conn;
    ...
    public function handle()
    {
        ...
        try {
            ...
            if (!$stmt2) {
                throw new Exception("Prepare failed for user lookup: " . $this->conn->error);
            }
            ...
            if (!$stmt3) {
                throw new Exception("Prepare failed for user update: " . $this->conn->error);
            }
            ...
            if (!$stmt4) {
                throw new Exception("Prepare failed for donor update: " . $this->conn->error);
            }
...
$handler = new MedicalVerificationHandler($conn);
$handler->handle();
